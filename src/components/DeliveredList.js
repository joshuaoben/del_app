...
//consume api to get orders based on agent id
	makeRequest = () => {
		this.setState({ loading: true });
		let api_url = 'http://healthnigeria.info/back/api/order/read.php?status=Delivered&agent=' + this.state.agent_id;
		axios.get(api_url)
...
//function to search through orders
	...
	renderHeader = () => {
	    ...
	    <>
	      ...
	      />
	      <Text style={{paddingLeft: 10, paddingTop: 10}}> DELIVERED ({this.state.data.length})</Text>
	      </>
	    );
	};
