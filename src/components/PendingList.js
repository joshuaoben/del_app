...
//consume api to get orders based on agent
	...
	makeRequest = () => {
		...
		let api_url = 'http://healthnigeria.info/back/api/order/read.php?status=Pending&agent=' + this.state.agent_id;
		axios.get(api_url)
...
//function to search through orders
	...
	renderHeader = () => {
	    ...
	    <>
	      ...
	      />
	      <Text style={{paddingLeft: 10, paddingTop: 10}}> PENDING ({this.state.data.length})</Text>
	      </>
	    );
	};
